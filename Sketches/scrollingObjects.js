...
function scrollPlatform(x,y,w,h) {
	...
	this.w = [w];
	this.h = [h];
	this.toCanvas = function(canvas) { //convert an array into canvas coordinates
		//if str == 'y', gives you y, str == 'x', gives you x
		//returns an array of the converted coordinates.
		//so far only x will be implemented
	...
	this.createPlatform = function(canvas) {
		...
		this.yf = mouseY;
		rect(this.xi-canvas.x,this.yi,this.xf-this.xi,this.yf-this.yi); //must subtract off canvasX before you draw
	}
	...
	this.inRect = function(p,canvas) {
		...
		for (var i = 0; i < this.x.length; i++) {
			if ( !((this.x[i] - canvas.x >= -this.w[i]) && (this.x[i] - canvas.x <= canvas.w)) ) {
				continue;
			}
			hitTest = ( (p[0] >= this.x[i]-canvas.x) && (p[0] <= this.x[i] - canvas.x + this.w[i]) ) && ( (p[1] >= this.y[i]) && (p[1] <= this.y[i] + this.h[i]) );
	...
	this.drawRect = function(canvas) {
		...
		for (var i = 0; i < this.x.length; i++) {
			if ( (this.x[i] - canvas.x >= -this.w[i]) && (this.x[i] - canvas.x <= canvas.w) )
				rect(this.x[i]-canvas.x,this.y[i],this.w[i],this.h[i]);
		}
	}
}
...
function scrollPlayer(x,y,d,canvas) {
	...
	this.y = y;
	this.d = d;
	this.maxX = (canvas.w)/2 + (canvas.w)/5;
	this.minX = (canvas.w)/2 - (canvas.w)/5;
